...
const DESCRIBE_TABLES = `
-- The postgrest query for describing available tables and their schemas.
-- FROM https://github.com/begriffs/postgrest/blob/57477749aa431559e38fd447272671db42f84c79/src/PostgREST/DbStructure.hs
SELECT DISTINCT
...
func describe_tables(d postgres_db) (results []map[string]interface{}, err error) {
	...
	rows, err := db.Query(DESCRIBE_TABLES)
	defer rows.Close()
	if err != nil {
		fmt.Fprintf(os.Stderr, "%v\n", err)
		return results, err
